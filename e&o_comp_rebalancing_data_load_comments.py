...
# Extract week number from the most recent file
week_number_sv_ci = int(most_recent_file.split("_wk_")[1].split(".")[0])

# Read the most recent file into pandas DataFrame
sv_ci = pd.read_csv(file_path)

# Read comments file
comments_file_path = "/Volumes/prod_catalog/shared_volume/gscr_ds/inventory_workstream/e&o_comp_rebalancing/input/input_comments/2024/"
comments_file = os.listdir(comments_file_path)  # Assuming there's only one file in the directory
most_recent_file_com = comments_file[-1]
comments_file_full_path = os.path.join(comments_file_path, most_recent_file_com)

# Extract week number from the comments file
week_number_comments = int(most_recent_file_com.split("_wk_")[1].split(".")[0])

# Check if the week numbers match
if week_number_sv_ci == week_number_comments:
    com = pd.read_csv(comments_file_full_path)
    # Proceed with further steps
    print("Week numbers match. Proceeding with further steps...")
    print(file_path)
    print(comments_file_full_path)      
else:
    # Read the file based on week_number_comments into pandas DataFrame
    file_path_comments = os.path.join(directory, f"sv_component_inventory_wk_{week_number_comments}.csv")
    sv_ci = pd.read_csv(file_path_comments)
    com = pd.read_csv(comments_file_full_path)    
    print("Week numbers don't match. Using sv_component_inventory_wk file same as comments file.")
    print(file_path_comments)
    print(comments_file_full_path)
...
week_number = int(most_recent_file_com.split("_wk_")[1].split(".")[0])
week_number

# COMMAND ----------

# %sql
# select * from prod_catalog.planning_cloud_views.supplier_visibility_2024_reports where week == 18
query = f"""
SELECT *
...
# sv_ci = pd.read_csv("/Volumes/prod_catalog/shared_volume/gscr_ds/inventory_workstream/e&o_comp_rebalancing/input/sv_component_inventory/2024/sv_component_inventory_wk_18.csv")
# #sv_dns = ps.read_delta('dbfs:/user/supplier_visibility_2024_reports')
# #sv_dns = sv_dns[sv_dns.week == 12]
# #alt_map = ps.read_delta('dbfs:/user/supp_part_alt_grps_mapping')
# com = pd.read_csv('/Volumes/prod_catalog/shared_volume/gscr_ds/inventory_workstream/e&o_comp_rebalancing/input/input_comments/2024/component_inventory_comments_wk_18.csv')
# #gsp = pd.read_csv('gs://its-managed-dbx-ds-01-d-user-work-area/inventory_workstream/eo_component_inventory_rebalancing/manual_input/gsp_names.csv')
...
com.rename(columns={'site_name':'site_name', 'part_no':'part_no', 'date':'date', 'gsp':'gsp', 'gsp_dept':'gsp_dept', 'comments':'comments','transferred_qty_actual':'transferred_qty_actual', 'transferred_qty_balance':'transferred_qty_balance'}, inplace=True)
...
columns_to_convert = ["demand_total", "po_qty","zoi","soi","hub","safety_stock","moq","raw_cost"]
...
sv_ci['zoi_soi_dem_qty'] = (sv_ci['zoi'] + sv_ci['soi'] - sv_ci['demand_total']).astype(float)
sv_ci['zoi_soi_dem_$'] = (sv_ci['zoi_soi_dem_qty'] * sv_ci['raw_cost']).astype(float)
sv_ci['zoi_soi_po_dem_qty'] = ((sv_ci['zoi'] + sv_ci['soi'] + sv_ci['po_qty']) - sv_ci['demand_total']).astype(float)
...
#transferred qty & comments - join the table with site_name * part no
#logic - for every part no * site name - get latest and last 2 records of comments and combine them into one line
# transferred qty & $ for last two times as 4 columns
# Append the Date and Comments columns
com['date'] = pd.to_datetime(com['date'], format='%m/%d/%Y')
com = com.sort_values('date', ascending=True)
com_prev = com[['site_name', 'part_no', 'date', 'comments']]

# Groupby Site Name and Supplier Part No and append the last 4 comments with their corresponding date
com_grouped = com_prev.groupby(['site_name', 'part_no'],group_keys=True)
com_last_4_comments = com_grouped['date', 'comments'].apply(lambda x: x.tail(5)).reset_index()
com_last_4_comments['date_comments'] = com_last_4_comments['date'].astype(str) + '|' + com_last_4_comments['comments'].astype(str)

# Combine all date_comments rows for each site_name and supplier part
com_combined = com_last_4_comments.groupby(['site_name', 'part_no'])['date_comments'].apply('|'.join).reset_index()
com_combined.columns = ['site_name', 'part_no', 'date_comments']

# To bring qty columns data
com_qty = com[['site_name','part_no','transferred_qty_actual','transferred_qty_balance']]
com_combined_1 = pd.merge(com_combined,com_qty, on=['site_name', 'part_no'],how='left')

#merging with final df
sv_ci_alt_dns_gsp_1_com = pd.merge(sv_ci_alt_dns_gsp_1,com_combined_1,on=['site_name', 'part_no'],how='left')

# COMMAND ----------

#transferred qty last two dates
#com_sup = com[com.supplier_part == '30-119104-02']

second_last_col = com[['site_name', 'part_no', 'transferred_qty_actual']].groupby(['site_name', 'part_no'], sort=False)['transferred_qty_actual'].apply(lambda x: x.iloc[-2] if len(x) > 2 else 0).reset_index()
second_last_col.rename(columns = {'transferred_qty_actual': 'second_last_trans_qty'}, inplace=True)

third_last_col = com[['site_name', 'part_no', 'transferred_qty_actual']].groupby(['site_name', 'part_no'], sort=False)['transferred_qty_actual'].apply(lambda x: x.iloc[-3] if len(x) > 3 else 0).reset_index()
third_last_col.rename(columns = {'transferred_qty_actual': 'third_last_trans_qty'}, inplace=True)

#merging with final df
sv_ci_alt_dns_gsp_1_com_trans1 = pd.merge(sv_ci_alt_dns_gsp_1_com, second_last_col, on=['site_name', 'part_no'], how='left')
sv_ci_alt_dns_gsp_1_com_trans2 = pd.merge(sv_ci_alt_dns_gsp_1_com_trans1, third_last_col, on=['site_name', 'part_no'], how='left')

# COMMAND ----------

# Get the week number from the input file name
filename = most_recent_file_com.split("_")[4]
week_number_comments = int(filename.split(".")[0])  # Assuming the file name format is "week_number.csv"
...
#Outputs to Csv --without comments
# Output file name with week number and current date
output_filename = f"eo_component_inventory_rebalancing_{week_number_comments}_{datetime.now().strftime('%m-%d-%Y')}.csv"
...
# # Cast 'oh' column to StringType
# spark_df = spark_df.withColumn("oh", col("oh").cast("double"))
...
#Outputs to Csv --with comments
# Output file name with week number and current date
output_filename = f"eo_component_inventory_rebalancing_comments_{week_number_comments}_{datetime.now().strftime('%m-%d-%Y')}.csv"

# Output file path
output_path = os.path.join("/Volumes/prod_catalog/shared_volume/gscr_ds/inventory_workstream/e&o_comp_rebalancing/output/", output_filename)

# Save DataFrame to CSV without comments
sv_ci_alt_dns_gsp_1_com_trans2.to_csv(output_path, index=False)

# COMMAND ----------

#create a spark dataframe
spark_df = spark.createDataFrame(sv_ci_alt_dns_gsp_1_com_trans2)

# COMMAND ----------

from pyspark.sql.functions import col

spark_df = spark_df.withColumn("transferred_qty_balance", col("transferred_qty_balance").cast("string"))

# COMMAND ----------

spark_df.write.mode("append").format("delta").saveAsTable("planning_cloud_views.eo_comp_rebalancing_data_load_com")

# COMMAND ----------

# # output as csv into GCS
# output_path = f"/Volumes/prod_catalog/shared_volume/gscr_ds/inventory_workstream/e&o_comp_rebalancing/output/eo_component_inventory_rebalancing_comments_18_051424.csv"
# sv_ci_alt_dns_gsp_1_com_trans2.to_csv(output_path,index=False)
...
# output_path = f"/Volumes/prod_catalog/shared_volume/gscr_ds/inventory_workstream/e&o_comp_rebalancing/output/eo_component_inventory_rebalancing_18_051424.csv"
# sv_ci_alt_dns_gsp_1.to_csv(output_path,index=False)
...
date_column = sv_ci_alt_dns_gsp_1_com_trans2['date']
date_column

# COMMAND ----------

transferred_qty = com.loc[com['transferred_qty_actual']>0,'transferred_qty_actual'].sum()
transferred_qty

# COMMAND ----------

com['transferred_qty_balance'] = com['transferred_qty_balance'].replace('$', '').replace(',', '')
com['transferred_qty_balance'] = pd.to_numeric(com['transferred_qty_balance'])
transferred_val = com.loc[com['transferred_qty_balance'] > 0, 'transferred_qty_balance'].sum()
transferred_val
...
# Update transferred_qty and transferred_val only if week numbers match
if week_number_sv_ci == week_number_comments:
    total_dict['transferred_qty_actual'] = transferred_qty
    total_dict['transferred_val_balance'] = transferred_val
    print('week number equal updating values')
else:
    # Get the week number from the comments_week_file
    current_sv_ci = int(file_path_comments.split("_wk_")[1].split(".")[0])

    # Check if the comments week number matches the week number in the comment file
    if current_sv_ci == week_number_comments:
        total_dict['transferred_qty_actual'] = transferred_qty
        total_dict['transferred_val_balance'] = transferred_val
...
# %sql
# CREATE TABLE planning_cloud_views.Reports_comments (
# Week BIGINT,
# Date TIMESTAMP,
# Total_ZOI_SOI_tot_excess_val_positive DOUBLE,
# Total_ZOI_SOI_opo_tot_excess_val_positive DOUBLE,
# OpportunityBalancing DOUBLE,
# transferred_qty_actual DOUBLE,
# transferred_val_balance DOUBLE
# );

# COMMAND ----------

total_df.write.format("delta").mode("append").option("mergeSchema", "true").insertInto("planning_cloud_views.Reports_comments")
...
# --        transferred_qty_actual,
# --        transferred_val_balance
# -- FROM planning_cloud_views.Reports_comments c
# -- WHERE NOT EXISTS (
# --     SELECT 1
# --     FROM planning_cloud_views.Reports_combined rc
# --     WHERE rc.Week = c.Week
# -- );

# -- Overwrite data in Reports_combined for weeks with matching data in Reports_comments
# INSERT into TABLE planning_cloud_views.Reports_combined
# SELECT c.Week, c.Date, 
#        c.Total_ZOI_SOI_tot_excess_val_positive, 
#        c.Total_ZOI_SOI_opo_tot_excess_val_positive, 
#        c.OpportunityBalancing,
#        c.transferred_qty_actual,
#        c.transferred_val_balance
# FROM planning_cloud_views.Reports_combined rc
# JOIN planning_cloud_views.Reports_comments c
# ON rc.Week = c.Week;

# COMMAND ----------

# MAGIC %sql
# MAGIC MERGE INTO planning_cloud_views.Reports_combined AS target
# MAGIC USING planning_cloud_views.Reports_comments AS source
# MAGIC ON target.Week = source.Week
# MAGIC WHEN MATCHED THEN
# MAGIC   UPDATE SET
# MAGIC     target.Date = source.Date,
# MAGIC     target.Total_ZOI_SOI_tot_excess_val_positive = source.Total_ZOI_SOI_tot_excess_val_positive,
# MAGIC     target.Total_ZOI_SOI_opo_tot_excess_val_positive = source.Total_ZOI_SOI_opo_tot_excess_val_positive,
# MAGIC     target.OpportunityBalancing = source.OpportunityBalancing,
# MAGIC     target.transferred_qty_actual = source.transferred_qty_actual,
# MAGIC     target.transferred_val_balance = source.transferred_val_balance;
